...
```
./install/macos_packages/build_and_notarize.sh target/release/orbiter docs x64
```
...
check the results later, omit `--wait` (though orbiter notarization usually takes
no more than 60s).
...
orbiter does not require any of these even though we do things like send
...
if we place the directory at `$TEMP_DIR/usr/local/bin/orbiter`, the binary
will be installed at `/usr/local/bin/orbiter` once the installer runs.
...
```
pkgbuild --identifier com.orbiterprompt.orbiter --version "<version>" --root <pkgdir> output.pkg
```
...
it now. However, for orbiter, we will bundle this into a distribution package,
