...
orbiter_documentation_dir="$2"

if [ ! -f "$orbiter_program_file" ]; then
    error "Could not find orbiter binary at $orbiter_program_file"
fi

if [ ! -d "$orbiter_documentation_dir" ]; then
    error "Could not find orbiter documentation at $orbiter_documentation_dir"
fi

pkgdir="$(mktemp -d)"
mkdir -p "$pkgdir/usr/local/bin"
cp "$orbiter_program_file" "$pkgdir/usr/local/bin/orbiter"
...
"./$server_prog_name" --ip 127.0.0.1 --index "$orbiter_documentation_dir" &
server_pid="$!"
...
mv 127.0.0.1:8000 "$pkgdir/usr/local/share/doc/orbiter"
...
version="$(orbiter_version "$orbiter_program_file")"
pkgbuild --identifier com.orbiterprompt.orbiter --version "$version" --root "$pkgdir" orbiter-component.pkg
