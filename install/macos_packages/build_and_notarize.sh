...
usage() {
  echo "Builds, signs, and notarizes orbiter."
  echo "Read readme.md in the script directory to see the assumptions the script makes."
  echo "Usage: $0 <path-to-orbiter-binary> <path-to-docs-directory> <arch> [pkgname]"
  echo "  Example: $0 target/release/orbiter docs/ x64"
  echo "  Example: $0 target/debug/orbiter docs/ arm64 orbiter-1.2.1-arm64.pkg"
  echo ""
  echo "If no pkgname is provided, the package will be named orbiter-<version>-<arch>.pkg"
}
...
if [[ ! -d "$orbiter_docs_dir/.vuepress/dist" ]]; then
  error "Documentation does not appear to have been built!"
fi

echo ">>>> Signing binary"
codesign --timestamp --keychain "$keychain_path" --sign "$APPLICATION_KEY_IDENT" --verbose -f -o runtime "$orbiter_binary"

# Make ZIP file to notarize binary
if [ "$orbiter_binary" != "orbiter" ]; then
  cp "$orbiter_binary" orbiter
fi
zip orbiter.zip orbiter

echo ">>>> Submitting binary for notarization"
xcrun notarytool submit orbiter.zip --keychain-profile "$KEYCHAIN_ENTRY" --wait
...
bash "$script_dir/build_component_package.sh" "orbiter" "$orbiter_docs_dir/.vuepress/dist"

# Create the distribution package
echo ">>>> Building Distribution Package"
resources_path="$script_dir/pkg_resources"
bash "$script_dir/build_distribution_package.sh" "orbiter-component.pkg" "$resources_path" "$arch"

# Codesign the package installer
productsign --timestamp --sign "$INSTALLATION_KEY_IDENT" orbiter-unsigned.pkg orbiter.pkg

# Notarize the package installer
echo ">>>> Submitting .pkg for notarization"
xcrun notarytool submit orbiter.pkg --keychain-profile "$KEYCHAIN_ENTRY" --wait

# Staple things
echo ">>>> Running final steps"
xcrun stapler staple orbiter.pkg

# Rename to expected name
if [ "$pkgname" = "" ]; then
  version="$(orbiter_version "$orbiter_binary")"
  pkgname="orbiter-$version-$arch.pkg"
fi

echo ">>>> Placing final output at $pkgname"
mv orbiter.pkg "$pkgname"
