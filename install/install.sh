...
download() {
  ...
  if has curl && curl_is_snap; then
    warn "curl installed through snap cannot download orbiter."
  ...
  printf "\n" >&2
  info "This is likely due to Orbiter not yet supporting your configuration."
...
unpack() {
  ...
  info "bug report at https://github.com/orbiter-rs/orbiter/issues"
  return 1
}
...
usage() {
  printf "%s\n" \
    "install.sh [option]" \
    "" \
    "Fetch and install the latest version of orbiter, if orbiter is already" \
...
install() {
  ...
  if test_writeable "${BIN_DIR}"; then
    sudo=""
    msg="Installing Orbiter, please wait…"
  else
    warn "Escalated permissions are required to install to ${BIN_DIR}"
    elevate_priv
    sudo="sudo"
    msg="Installing Orbiter as root, please wait…"
  fi
  info "$msg"
...
print_install() {
  ...
  do
    ...
    config_file="~/.${s}rc"
    config_cmd="eval \"\$(orbiter init ${s})\""
    ...
    case ${s} in
      ...
      fish )
        ...
        config_cmd="orbiter init fish | source"
        ;;
      tcsh )
        config_cmd="eval \`orbiter init ${s}\`"
        ;;
      xonsh )
        config_cmd="execx(\$(orbiter init xonsh))"
        ;;
    esac
  ...
  do
    ...
    case ${s} in
      elvish )
        ...
        config_cmd="eval (orbiter init elvish)"
      ...
      nushell )
        ...
        config_cmd="use ~/.cache/orbiter/init.nu"
  ...
  Add the following to the end of ${BOLD}your Nushell env file${NO_COLOR} (find it by running ${BOLD}\$nu.env-path${NO_COLOR} in Nushell):
	...
	mkdir ~/.cache/orbiter
	orbiter init nu | save -f ~/.cache/orbiter/init.nu"
        ;;
    esac
  ...
  Typically the path is ~\Documents\PowerShell\Microsoft.PowerShell_profile.ps1 or ~/.config/powershell/Microsoft.PowerShell_profile.ps1 on -Nix." \
    "Invoke-Expression (&orbiter init powershell)"
  ...
  printf "  %s\n  You need to use Clink (v1.2.30+) with Cmd. Add the following to a file %s and place this file in Clink scripts directory:\n\n\t%s\n\n" \
    ...
    "${BOLD}orbiter.lua${NO_COLOR}" \
    "load(io.popen('orbiter init cmd'):read(\"*a\"))()"
...
is_build_available() {
  ...
  if [ "${good}" != "1" ]; then
    error "${arch} builds for ${platform} are not yet available for Orbiter"
    printf "\n" >&2
    info "If you would like to see a build for your configuration,"
    info "please create an issue requesting a build for ${MAGENTA}${target}${NO_COLOR}:"
    info "${BOLD}${UNDERLINE}https://github.com/orbiter-rs/orbiter/issues/new/${NO_COLOR}"
    printf "\n"
    exit 1
  fi
}
...
if [ -z "${BASE_URL-}" ]; then
  BASE_URL="https://github.com/orbiter-rs/orbiter/releases"
fi
...
URL="${BASE_URL}/latest/download/orbiter-${TARGET}.${EXT}"
info "Tarball URL: ${UNDERLINE}${BLUE}${URL}${NO_COLOR}"
confirm "Install Orbiter ${GREEN}latest${NO_COLOR} to ${BOLD}${GREEN}${BIN_DIR}${NO_COLOR}?"
check_bin_dir "${BIN_DIR}"

install "${EXT}"
completed "Orbiter installed"
